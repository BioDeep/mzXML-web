    ...
    (function (MSMSViewer) {
        function tooltip(mz) {
            var tip = d3.tip()
                .attr("class", "d3-tip")
                .offset([-10, 0])
                .html(function (d) { return mz.tooltip(d); });
            return tip;
        }
        ...
        function mzrtTip() {
            var tip = d3.tip()
                .attr("class", "d3-tip")
                .offset([-10, 0])
                .html(function (d) { return d.precursor_mass + "@" + d.rt; });
            return tip;
        }
    ...
    (function (MSMSViewer) {
        ...
        function renderChart(containerId, api, id, decoder) {
            ...
            $ts.get(url, function (result) {
                if (result.code == 0) {
                    var data_1 = MSMSViewer.Data.JSONParser((result.info), decoder);
                    var d3_1 = new MSMSViewer.d3Renderer(data_1, size);
                    d3_1.rendering(containerId);
                }
                else {
                    // 显示错误消息
        ...
        */
        function previews(divId, data, size, title, margin, csvLink) {
            if (size === void 0) { size = [900, 600]; }
            if (title === void 0) { title = BioDeep.MSMSViewer.title; }
            ...
            if (data instanceof BioDeep.IO.mgf) {
                return new MSMSViewer.d3Renderer(parseIon(data), size, margin, csvLink, title).rendering(divId);
            }
            else {
                return new MSMSViewer.d3Renderer(data, size, margin, csvLink, title).rendering(divId);
            }
        }
        ...
        (function (Data) {
            var mzData = /** @class */ (function () {
                function mzData(mz, align) {
                    var range = Array.isArray(mz) ? data.NumericRange.Create(mz) : mz;
                    var d = range.min * 0.2;
                    if (range.Length == 0.0) {
            ...
            */
            function chartting(engine) {
                ...
                engine.svg.selectAll(".bar")
                    ...
                    .enter()
                    .append("rect")
                    .attr("class", function (d) { return "bar " + ((d.into < 0) ? "negative" : "positive"); })
                ...
                //柱状图顶部参数
                var dataset = [];
                for (var i = 0; i < engine.input.mzMatrix.length; i++) {
                ...
                engine.svg.selectAll(".MyText")
                    ...
                    .attr("font-size", '10px')
                    .text(function (d) { return d.mz.toFixed(2); });
                engine.svg.append("g")
                    .attr("class", "x axis")
                    .call(yAxis);
                engine.svg.append("g")
            ...
            function legend(engine, title) {
                ...
                if (/(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent) || /(Android)/i.test(navigator.userAgent)) {
                    ...
                    var width = CanvasHelper.getTextWidth(title, font);
...
(function (BioDeep) {
    ...
    (function (MSMSViewer) {
        var d3Renderer = /** @class */ (function (_super) {
            ...
            function d3Renderer(mz, size, margin, csvLink, title) {
                ...
                _this.margin = margin;
                _this.csvLink = csvLink;
                _this.title = title;
                _this.strokeWidth = 6;
                _this.radius = 6;
                _this.current = mz.trim().normalize();
                _this.registerDownloader(_this.csvLink);
                return _this;
            }
            ...
            d3Renderer.prototype.registerDownloader = function (id) {
                var a = $ts(Internal.Handlers.makesureElementIdSelector(id));
            ...
            d3Renderer.prototype.unique = function (arr) {
                return Array.from(new Set(arr));
            };
            d3Renderer.prototype.toId = function (div) {
            ...
            */
            d3Renderer.prototype.rendering = function (div) {
                ...
                MSMSViewer.renderingWork.legend(this, this.title);
                this.tip.hide();
            };
            return d3Renderer;
        }(SvgChart));
    ...
    (function (MSMSViewer) {
        ...
        */
        var TICplot = /** @class */ (function (_super) {
            ...
            TICplot.prototype.plotData = function (canvas, rt, intensity, tags) {
                BioDeep.MSMSViewer.clear(canvas);
            ...
            TICplot.prototype.bindEvents = function (ticks) {
                ...
                var vm = this;
                this.ions = ticks.ToDictionary(function (x) { return TICplot.uniqueId(x); }, function (x) { return x.raw; });
            ...
            TICplot.prototype.chart = function (canvas) {
                ...
                svg.selectAll("mzrt-dot")
                    ...
                    .attr("tooltip", function (d) { return TICplot.uniqueId(d); })
