namespace BioDeep.Models {
    ...
    */
    export class mzInto {
         ...
         *             显示所需要用到的原始的质谱信号强度数据
        */
        public constructor(public id: string, public mz: number, public into: number) { }
    ...
    export function SpectrumMatrix(data: IEnumerator<IO.mgf>, levels: number): IEnumerator<mzInto> {
        if (levels == SpectrumLevels.MS1) {
            // only ms1
            return data.Select(ion => new mzInto("", parseFloat(ion.precursor_mass.toString()), ion.intensity));
            ...
            return data.Select(function (ion) {
                ...
                let ms1 = new mzInto("", parseFloat(ion.precursor_mass.toString()), ion.intensity);
